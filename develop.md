...
第二次実装：
* L: SD カードルート上の WAV ファイルリストを表示。表示のみ。リストの最終行は '.' 
* F fname.wav: 現在選択ファイル名を指定（設定ファイルに書き込む）。存在しないファイル名はエラー。成功・エラーを短い応答 'OK' / 'FAIL' で返す
* S: （実装内容変更）設定プロパティをリスト表示。表示のみ

第三次実装:
* I: is_playing? Y/N で返す

第四時実装:
* Z: M5Cardputer をリセット - 機器を既知状態に戻す

現行方針:

* シリアル入力だけを厳密コマンド化する
* 有効コマンドは `P` `R` `S` `I` `Z` `C` `M` `N` `L` `F fname.wav`
* `M` / `N` は `use_rmt` を即保存する
* `L` と `S` はシリアルへテキスト応答を返すだけで、UI 状態は変えない
* `I` は `is_playing` を `Y` / `N` で返す
* `Z` は `RESET` を返してから再起動する
* `F fname.wav` は改行で確定し、成功時に `current_file` を保存して `OK` / 失敗時に `FAIL` を返す
* キーボードと BtnA の既存挙動は維持する
* Browse / Settings 内のシリアル操作は今回未対応